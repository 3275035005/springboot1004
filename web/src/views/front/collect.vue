    ...
    <el-table :data="tableData" border stripe style="width: 100%; margin-top: 10px">
      <el-table-column label="宠物名称">
        <template slot-scope="scope">
          <a :href="['/front/goods?id=' + scope.row.petId]">{{ scope.row.petName }}</a>
        </template>
      </el-table-column>
      <el-table-column prop="goodsImg" label="宠物图片">
        <template slot-scope="scope">
          <el-image :src="scope.row.petImg" style="width: 100%; height: 100px" fit="contain"></el-image>
        </template>
      </el-table-column>
